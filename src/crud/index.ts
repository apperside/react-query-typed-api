...
type BasePath<T extends string | unknown = unknown> = `${T extends string
  ? T
  : ''}${T extends string ? '/' : ''}`;
 ...
 */
export type RoutesForModel<
  CrudModel extends keyof RoutesModelMapping,
...
> = {
  [key in AllRoutes<CrudModel, BasePath>]: key extends `${string}/bulk`
    ? {
        responseType: ActionTypesMapping['saveManyResponse'] extends Record<
          string,
          any
        >
          ? ActionTypesMapping['saveManyResponse']
          : DefaultGetManyResponse<RoutesModelMapping[CrudModel]>;
        payloadType: ActionTypesMapping['saveManyPayload'] extends Record<
          string,
          any
        >
          ? ActionTypesMapping['saveManyPayload']
          : DefaultSaveManyPayload<RoutesModelMapping[CrudModel]>;
      }
    : key extends `${string}/:id`
    ? {
        responseType: ActionTypesMapping['getOneResponse'] extends Record<
          string,
          any
        >
          ? ActionTypesMapping['getOneResponse']
          : DefaultGetOneResponse<RoutesModelMapping[CrudModel]>;
        payloadType: ActionTypesMapping['saveOnePayload'] extends Record<
          string,
          any
        >
          ? ActionTypesMapping['saveOnePayload']
          : DefaultSaveOnePayload<RoutesModelMapping[CrudModel]>;
      }
    : key extends `${string}/bulk-delete/:ids`
    ? {
        responseType: ActionTypesMapping['deleteManyResponse'] extends Record<
          string,
          any
        >
          ? ActionTypesMapping['deleteManyResponse']
          : DefaultDeleteManyResponse<RoutesModelMapping[CrudModel]>;
        payloadType: ActionTypesMapping['deleteManyPayload'] extends Record<
          string,
          any
        >
          ? ActionTypesMapping['deleteManyPayload']
          : DefaultDeleteManyPayload<RoutesModelMapping[CrudModel]>;
      }
    : key extends `${string}`
    ? {
        responseType: ActionTypesMapping['getManyResponse'] extends Record<
          string,
          any
        >
          ? ActionTypesMapping['getManyResponse']
          : DefaultGetManyResponse<RoutesModelMapping[CrudModel]>;
        payloadType: ActionTypesMapping['saveOnePayload'] extends Record<
          string,
          any
        >
          ? ActionTypesMapping['saveOnePayload']
          : DefaultSaveOnePayload<RoutesModelMapping[CrudModel]>;
        mutationResponseType: ActionTypesMapping['saveOneResponse'] extends Record<
          string,
          any
        >
          ? ActionTypesMapping['saveOneResponse']
          : DefaultSaveOneResponse<RoutesModelMapping[CrudModel]>;
      }
...
export type NestJsxModelRoute<
  T extends keyof RoutesModelMapping,
  BasePath extends string | unknown = unknown
> = RoutesForModel<T, BasePath, NestCrudResponseTypes<RoutesModelMapping[T]>>;

/**
 * @deprecated
 * use {@link NestJsxModelRoute} instead
 */
export type NestModelRoute<
  T extends keyof RoutesModelMapping,
  BasePath extends string | unknown = unknown
> = NestJsxModelRoute<T, BasePath>;
